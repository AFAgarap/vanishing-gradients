...
#    max_gradients = [tf.reduce_min(gradient) for gradient in gradients]
#    if tf.reduce_min(max_gradients).numpy() < 1e-3:
#    gradients = [tf.add(gradient, tf.random.normal(stddev=5e-2, mean=0., shape=gradient.shape)) for gradient in gradients]
    gradients = [tf.add(gradient, gradient * 5e-1) for gradient in gradients]
...
def train(model, loss_fn, dataset, epochs=10):
    ...
    writer = tf.summary.create_file_writer('tmp/{}-relu-injection-fmnist'.format(time.asctime()))
